...
RSpec.describe 'sprout-postgresql' do
  ...
  it 'runs postgres on the default port' do
    ...
    expect(system('psql -c "select 1" &> /dev/null')).to eq(true)
  end
  ...
  it 'is managed by launchd' do
    expect(system('launchctl list homebrew.mxcl.postgresql &> /dev/null')).to eq(true)
  end

  it 'creates a database for the current user' do
    db_name = ENV['USER']
    expect(system("psql -U #{db_name} -c 'select 1' #{db_name} &> /dev/null")).to eq(true)
  end

  it 'creates a postgres user' do
    expect(system('psql -U postgres -c "select 1" &> /dev/null')).to eq(true)
  end
end
